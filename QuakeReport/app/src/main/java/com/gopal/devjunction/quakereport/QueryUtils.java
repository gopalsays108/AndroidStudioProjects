...
public class QueryUtils {
     ...
     */
    public static ArrayList<EarthquakeModel> extractEarthquake(String string) {
        Log.i( LOG_TAG, "extractEarthquake is called : " );

        URL url2 = createUrl( string );
        String jsonResponse = "";
...
//        }
        ...
        if (jsonResponse != null)
            try {
                ...
                JSONArray jsonArray = rootObject.optJSONArray( "features" );

                String direction = "";
                String actual = "";
                if (jsonArray != null) {
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject finalObject = jsonArray.optJSONObject( i ).optJSONObject( "properties" );
                        //We can teo optJsonTogether or we can also do as follow
                        //JSONObject properties = finalObject.optJSONObject("properties); and then use properties variable;
                        if (finalObject != null) {

                            double mag = finalObject.getDouble( "mag" );
                            String place = finalObject.getString( "place" );
                            String time = String.valueOf( finalObject.getLong( "time" ) );
...
//                        }
                            ...
                            earthquakeModels.add( new EarthquakeModel( mag, actual, date, direction, url ) );
                        }
                    }
                }
    ...
    private static String readFromStream(InputStream inputStream) throws IOException {
        ...
        if (inputStream != null) {
            ...
            //t hat is it knows how to decode each byte in human readable character that is a, b,c
